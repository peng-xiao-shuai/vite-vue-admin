 ...
 * @Last Modified time: 2023-02-27 18:57:57
 */
 ...
 */
export const addLists = (key: string, val: any, replace = false) => {
  // 生成id
  val.id = lists[key].length + 1
  val.key = lists[key].length + 1
  const date = new Date()
  val.createTime = `${date.getFullYear()}-${
...
export const deleteLists = (key: string, ids: any[]) => {
  ...
  copy.forEach((item: any, index: number) =>
    ids.indexOf(item.id) !== -1 ? lists[key].splice(index, 1) : ''
  )
}
