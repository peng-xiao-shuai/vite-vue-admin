...
function recursion(each: Routers, childs: rolesValueItemType[]) {
  ...
  if (!each.children) {
    ...
    if (each.meta && (each.meta as Meta).id) {
      ids = childs.filter(
        (i: rolesValueItemType) => (each.meta as Meta).id == i.parentId
      )
    ...
    if (ids.length > 0) {
      ...
      for (const childrenItem of each.children) {
        const arr = addRouterFun(ids, childrenItem)
        if (arr) {
          children.push(arr)
...
const actions = {
  ...
  loginAction(
    ...
    user: { username: string; password: number }
  ) {
  ...
  // 登出
  outLogin(store: Store<UserState>) {
  ...
  tagsActions(
    store: Store<UserState>,
    val: { to: Tags; removeIndex?: number[]; name?: string }
  ) {
...
const mutations = {
  ...
  tagsCommit(
    ...
    if (val.removeIndex !== undefined || val.name !== undefined) {
      ...
      val.removeIndex &&
        state.tags.splice(val.removeIndex[0], val.removeIndex[1])
    } else {
