<template>
  <div class="search-view">
    <div class="wrap">
      ...
      <div class="tip">
        {{ t('you.can.use.shortcut.keys') }} <span>s</span>
        {{ t('call.up.the.search.panel,press') }} <span>esc</span>
        {{ t('close') }}
      </div>
    </div>
    ...
    >
      <el-scrollbar>
        <el-empty
          description="抱歉，没有找到相关页面！！"
          v-show="!funMenu().length"
        />

        <template v-for="item in funMenu()">
          <div
            v-if="item.meta && item.meta.locale"
            :key="item.meta.locale"
            :label="item.path"
            :value="item.name"
            class="menu-item"
            @click="navTo(item)"
...
const searchMenusFun = (arr: Arr[], menu: Arr[], superior?: Arr) => {
  arr.forEach((each: Arr) => {
    ...
    if ((!item.hidden as boolean) && item.name) {
      item.locales =
        superior && superior.locales
          ? superior.locales.concat([(item.meta as { locale: string }).locale])
...
const funMenu = () => {
  ...
  return menus.filter((item: Arr) => {
    return (
      item.path.indexOf(searchValue.value) != -1 ||
      (item.meta &&
        item.meta.locale &&
        t(item.meta.locale as string).indexOf(searchValue.value) != -1)
    )
  })
}
