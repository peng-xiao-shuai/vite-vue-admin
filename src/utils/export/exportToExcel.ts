...
export function export_json_to_excel_sheet(
  ...
  for (let tmp = 0; tmp <= datas.length - 1; tmp++) {
    // 获取底部sheet名称
    const ws_name = sheetNames[tmp] || 'sheet' + (tmp + 1)
    ...
    // 使用默认标题合并
      ...
      if (multiHeaders[tmp] && multiHeaders[tmp].length) {
        multiHeaders[tmp].forEach((mhItem: any, mhIndex: number) => {
          ...
          ws.mergeCells(
            'A' + (mhIndex + 1),
            createCol(header.length - 1) + (mhIndex + 1)
          )
    ...
    rowCenter(
      ws,
      header.length ? header : multiHeaders[tmp],
      data,
      dataLength,
      tmp
    )
    ...
    const autoWidthFlag: boolean =
      autoWidths[tmp] != undefined
        ? autoWidths[tmp]
        : configData.autoWidth == undefined
        ? true
        : configData.autoWidth
    // 自动处理单元格列宽
    if (autoWidthFlag) {
      // 设置worksheet每列的最大宽度
      const columnWidth = data.map((row: any) =>
        row.map((val: any) => {
  ...
  function rowCenter(
    ...
    header.forEach((hItem: any, hIndex: number) => {
      for (let dIndex = 0; dIndex < dataLength; dIndex++) {
        ...
        arg_ws.getCell(createCol(hIndex) + (dIndex + 1)).alignment = {
          vertical: 'middle',
          horizontal: alignFlag,
        }
        // 是否需要黑色边框，默认需要
        // 是否需要黑色边框设置的优先等级，局部 -> 全局 -> 默认
        const borderFlag: boolean =
          borders[iIndex] != undefined
            ? borders[iIndex]
            : configData.border == undefined
            ? true
            : configData.border
        ...
        const stripeFlag: boolean =
          stripes[iIndex] != undefined
            ? stripes[iIndex]
            : configData.stripe
            ? true
            : false
